  ...
  const handleRazorpayUpgrade = async () => {
    ...
    await initiateRazorpayPayment({
      ...
      onSuccess: (data) => {
        ...
        setSuccess(`Payment successful! Razorpay Payment ID: ${data.paymentId}. Your Pro subscription is now active.`);
        fetchData();
      },
      onError: (err) => {
        setActionLoading(false);
        setError(err.message || "Payment process encountered an error.");
      },
      onDismiss: () => {
        setActionLoading(false);
      }
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "32px" }}>
        <div>
          ...
          <p style={{ color: "var(--text-secondary)", fontSize: "0.95rem" }}>
            Manage your plan, check renewal dates, and view your transaction history.
          </p>
        </div>
        ...
        <div className="card" style={{ padding: "32px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "24px" }}>
          <div>
            ...
            <h2 style={{ fontSize: "1.75rem", fontWeight: 800, color: "var(--text-primary)", marginTop: "12px", marginBottom: "6px" }}>
              {subStatus?.is_subscribed ? "₹199 / Month" : "Free Account"}
            </h2>
          ...
          <div style={{ display: "flex", gap: "12px" }}>
            {subStatus?.is_subscribed ? (
              ...
              <button
                id="billing-upgrade-razorpay-btn"
                disabled={actionLoading}
                onClick={handleRazorpayUpgrade}
                className="btn btn-primary"
                style={{ padding: "10px 20px" }}
              >
                {actionLoading ? "Processing..." : "Upgrade to Pro (Razorpay)"}
              </button>
            )}
          </div>
        </div>
