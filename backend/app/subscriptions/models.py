...
class Plan(SQLModel, table=True):
    ...
    monthly_price: Decimal = Field(default=Decimal("0.00"), sa_column=Column(Numeric(10, 2), nullable=False))
...
class Subscription(SQLModel, table=True):
    ...
    plan_id: str = Field(foreign_key="plans.id", nullable=False, ondelete="CASCADE")
...
class Payment(SQLModel, table=True):
    ...
    subscription_id: Optional[str] = Field(default=None, foreign_key="subscriptions.id", nullable=True, ondelete="SET NULL")
    provider: str = Field(default="stripe")
