import io
import av
import httpx
import asyncio
import traceback
from abc import ABC, abstractmethod
from app.config import WHISPER_API_KEY  # loaded as GROQ_API_KEY or WHISPER_API_KEY in backend config

def log_debug(msg: str):
    try:
        with open("c:/Users/hites/Desktop/AI Int/backend/voice_debug.log", "a", encoding="utf-8") as f:
            f.write(msg + "\n")
    except Exception as e:
        print(f"Failed to write to debug log: {e}")

def webm_to_wav(webm_bytes: bytes) -> bytes:
    """
    Decodes input audio bytes (e.g. WebM/Opus) and resamples to 16kHz mono PCM WAV.
    Falls back to original bytes if decoding fails.
    """
    log_debug(f"--- webm_to_wav called with {len(webm_bytes)} bytes ---")
    try:
        input_file = io.BytesIO(webm_bytes)
        container = av.open(input_file)
        
        # Check if there is an audio stream
        if not container.streams.audio:
            log_debug("[WARNING] No audio stream found in input bytes.")
            return webm_bytes
            
        resampler = av.AudioResampler(
            format='s16',
            layout='mono',
            rate=16000
        )
        
        out_file = io.BytesIO()
        out_file.write(b'\x00' * 44)  # Placeholder for WAV header
        
        data_size = 0
        for frame in container.decode(audio=0):
            resampled_frames = resampler.resample(frame)
            for resampled_frame in resampled_frames:
                for plane in resampled_frame.planes:
                    chunk = bytes(plane)
                    out_file.write(chunk)
                    data_size += len(chunk)
                    
        log_debug(f"Decoded data size: {data_size} bytes")
        if data_size == 0:
            log_debug("[WARNING] Decoded data size is 0.")
            return webm_bytes
            
        # Go back to start and write WAV header
        out_file.seek(0)
        out_file.write(b'RIFF')
        out_file.write((36 + data_size).to_bytes(4, 'little'))
        out_file.write(b'WAVEfmt ')
        out_file.write((16).to_bytes(4, 'little'))
        out_file.write((1).to_bytes(2, 'little'))  # PCM
        out_file.write((1).to_bytes(2, 'little'))  # mono
        out_file.write((16000).to_bytes(4, 'little'))  # 16kHz
        out_file.write((32000).to_bytes(4, 'little'))  # byte rate (16000 * 2)
        out_file.write((2).to_bytes(2, 'little'))  # block align
        out_file.write((16).to_bytes(2, 'little'))  # bits
        out_file.write(b'data')
        out_file.write(data_size.to_bytes(4, 'little'))
        
        return out_file.getvalue()
    except Exception as e:
        err_msg = traceback.format_exc()
        log_debug(f"[WARNING] Failed to decode WebM to WAV using PyAV: {e}\n{err_msg}")
        return webm_bytes

class SpeechToTextService(ABC):
    @abstractmethod
    async def transcribe(self, audio_bytes: bytes) -> str:
        """
        Transcribes the given audio bytes into text.
        """
        pass

class GroqWhisperService(SpeechToTextService):
    def __init__(self, api_key: str):
        self.api_key = api_key
        self.url = "https://api.groq.com/openai/v1/audio/transcriptions"
        
    async def transcribe(self, audio_bytes: bytes) -> str:
        if not self.api_key:
            raise ValueError("Groq API key is not configured on the backend.")
            
        log_debug(f"--- transcribe called with {len(audio_bytes)} bytes ---")
        
        # 1. Convert input audio bytes to PCM WAV (16kHz mono) if not already WAV
        if audio_bytes.startswith(b'RIFF'):
            wav_bytes = audio_bytes
            log_debug(f"Input is already a WAV file, skipping webm_to_wav. Size: {len(wav_bytes)} bytes")
        else:
            wav_bytes = webm_to_wav(audio_bytes)
        
        # Determine content type and filename based on conversion success
        if wav_bytes.startswith(b'RIFF'):
            filename = "speech.wav"
            content_type = "audio/wav"
            log_debug(f"Sending WAV audio: {len(wav_bytes)} bytes")
        else:
            filename = "speech.webm"
            content_type = "audio/webm"
            log_debug(f"Sending WebM audio (fallback): {len(wav_bytes)} bytes")
            
        headers = {
            "Authorization": f"Bearer {self.api_key}"
        }
        
        files = {
            "file": (filename, wav_bytes, content_type)
        }
        
        data = {
            "model": "whisper-large-v3",
            "temperature": "0.0",
            "language": "en"
        }
        
        # Timeout is 10.0 seconds
        timeout = httpx.Timeout(10.0)
        
        # Retry loop (up to 2 retries)
        last_exception = None
        for attempt in range(1, 4):  # Attempt 1, 2, 3 (2 retries)
            try:
                log_debug(f"[ASR] Sending to Groq (Attempt {attempt})...")
                async with httpx.AsyncClient() as client:
                    response = await client.post(
                        self.url,
                        headers=headers,
                        files=files,
                        data=data,
                        timeout=timeout
                    )
                
                if response.status_code == 200:
                    result = response.json()
                    transcript = result.get("text", "").strip()
                    log_debug(f"[ASR] Groq Response received: '{transcript}'")
                    return transcript
                else:
                    err_detail = response.text
                    log_debug(f"[ASR] Groq error code {response.status_code}: {err_detail}")
                    # If rate limited (429) or server error (5xx), we raise to retry
                    if response.status_code in [429, 500, 502, 503, 504]:
                        raise httpx.HTTPStatusError(
                            f"HTTP {response.status_code}", 
                            request=response.request, 
                            response=response
                        )
                    else:
                        raise ValueError(f"Groq API returned error {response.status_code}: {err_detail}")
            except (httpx.RequestError, httpx.HTTPStatusError) as e:
                log_debug(f"[ASR] Attempt {attempt} failed with exception: {e}")
                last_exception = e
                if attempt < 3:
                    # Exponential backoff: 0.5s, 1.0s
                    backoff = 0.5 * (2 ** (attempt - 1))
                    await asyncio.sleep(backoff)
            except Exception as e:
                log_debug(f"[ASR] Unexpected error: {e}")
                raise e
                
        log_debug(f"[ASR] All transcription attempts failed. Last exception: {last_exception}")
        raise last_exception or Exception("ASR Transcription failed after retries.")

# Instantiate global service using the configured environment key
speech_to_text_service = GroqWhisperService(WHISPER_API_KEY)
